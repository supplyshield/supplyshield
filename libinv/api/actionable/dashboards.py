...
def _render_actionable_dashboard(include_epss: bool):
        ...
        for package in actionable_packages:
            ...
            result = {
                ...
                "latest_version_score": (
                    latest_version.score if latest_version else None
                ) if include_epss else latest_version.score,
                "suggested_versions": secure_versions,
                "versionless_id": package.available_version.actionable.uuid,
            }

            if include_epss:
                result["current_version_epss_score"] = package.available_version.epss_score
                result["epss_score"] = package.available_version.epss_score  # For template compatibility
                result["latest_version_epss_score"] = (
    ...
    if include_epss:
        epss_flag = any(result.get("epss_score") is not None for result in results)
    else:
        epss_flag = False
    ...
    return render_template(
        ...
        jenkins_url=jenkins_url,
        build_timestamp=build_timestamp,
        include_epss=epss_flag,
    )


@actionable.route("/v2/", methods=["GET"])
def actionables_v2():
    return _render_actionable_dashboard(include_epss=False)


@actionable.route("/v3/", methods=["GET"])
def actionables_v3():
    return _render_actionable_dashboard(include_epss=True)
