...
def scan_image_index(image_index: ImageIndex, account_id: str):
    ...
    # Sprint 27: Histogram.time() records the elapsed wall-clock time on
    # context exit regardless of exception, so both success and failure
    # paths populate the histogram. The try/except below remains the
    # Sprint 26 failure-counter contract — duration recording wraps it.
    with scan_duration_seconds.labels(type="image").time():
        try:
            for image_tar in image_index.pull_images_if_not_exist():
                ...
                with session_scope() as session:
                    ...
                    save_layer_information_for_image(
                        session=session, image=image, image_tar=image_tar
                    )
